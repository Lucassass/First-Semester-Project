...
import com.SemesterProject.iceandfire.Config;
import com.SemesterProject.iceandfire.DealObsolete;
import com.SemesterProject.iceandfire.DealCategory;
import com.SemesterProject.iceandfire.ItemObsolete;
import com.SemesterProject.iceandfire.data.InventoryData;
...
public class Inventory {
    ...
    private IInventoryData iInventoryData = new InventoryData();
    ...
    {   // If inventory is not filled, max 3, add to inventory, remove from old and print text
        if(iInventoryData.getItemSize() != maxItem && iInventoryData.getItemSize() < maxItem )
        {
            inventoryItem.add(itemObsolete);
        ...
        else if (iInventoryData.getItemSize() == maxItem)
        {
                ...
                if (scanIn.equalsIgnoreCase("yes"))
                {
                    ...
                    for (int i = 0; i < inventoryItem.size(); i++)
                    {
                        ...
                        if (scanYes.equals(inventoryItem.get(i).getName())) //goes through inventory
                        {
                            ...
                            inventoryItem.set(i, itemObsolete);
                            country.remove(itemObsolete);
                            n++;
                        }
                        else
    ...
    void printInventoryItem(ArrayList<ItemObsolete> list)
    {
        ...
        {   if(i< list.size()-1)
        {
            System.out.print(getAllItems().get(i).getName() + ", ");
        }
        else
        {
            System.out.println(getAllItems().get(i).getName());
        }
        }
    }
